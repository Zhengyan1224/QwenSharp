...
/// <summary>
/// Qwen2 Model with a token classification head on top (a linear layer on top of the hidden-states output).
/// e.g. for Named-Entity-Recognition (NER) tasks.
/// </summary>
public class Qwen2ForTokenClassification : nn.Module
{
    private readonly Qwen2Model _model;
    private readonly Dropout _dropout;
    private readonly Linear _score;

    public Qwen2ForTokenClassification(Qwen2Config config, string name = "model") : base(name)
    {
    ...
    public static Qwen2ForTokenClassification FromPretrained(string modelDirectory)
    {
        var config = ModelConfig.FromDirectory<Qwen2Config>(modelDirectory);
        var model = new Qwen2ForTokenClassification(config);
