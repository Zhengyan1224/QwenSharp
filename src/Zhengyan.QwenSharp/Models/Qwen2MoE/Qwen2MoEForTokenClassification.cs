...
/// <summary>
/// Qwen2MoE Model with a token classification head on top (a linear layer on top of the hidden-states output).
/// e.g. for Named-Entity-Recognition (NER) tasks.
/// </summary>
public class Qwen2MoEForTokenClassification : nn.Module
{
    private readonly Qwen2MoEModel _model;
    private readonly Dropout _dropout;
    private readonly Linear _score;

    public Qwen2MoEForTokenClassification(Qwen2MoEConfig config, string name = "model") : base(name)
    {
    ...
    public static Qwen2MoEForTokenClassification FromPretrained(string modelDirectory)
    {
        var config = ModelConfig.FromDirectory<Qwen2MoEConfig>(modelDirectory);
        var model = new Qwen2MoEForTokenClassification(config);
