...
/// <summary>
/// Qwen3MoE Model with a sequence classification/regression head on top (a linear layer on top of the pooled output).
/// e.g., for GLUE tasks.
/// </summary>
public class Qwen3MoEForSequenceClassification : nn.Module
{
    private readonly Qwen3MoEModel _model;
    private readonly Linear _score;

    public Qwen3MoEForSequenceClassification(Qwen3MoEConfig config, string name = "model") : base(name)
    {
    ...
    public static Qwen3MoEForSequenceClassification FromPretrained(string modelDirectory)
    {
        var config = ModelConfig.FromDirectory<Qwen3MoEConfig>(modelDirectory);
        var model = new Qwen3MoEForSequenceClassification(config);
