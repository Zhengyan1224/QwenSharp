...
/// <summary>
/// Qwen3 Model with a span classification head on top for extractive question-answering 
/// tasks like SQuAD (a linear layer on top of the hidden-states output to compute span start and end logits).
/// </summary>
public class Qwen3ForQuestionAnswering : nn.Module
{
    private readonly Qwen3Model _model;
    private readonly Linear _qaOutputs;

    public Qwen3ForQuestionAnswering(Qwen3Config config, string name = "model") : base(name)
    {
    ...
    public static Qwen3ForQuestionAnswering FromPretrained(string modelDirectory)
    {
        var config = ModelConfig.FromDirectory<Qwen3Config>(modelDirectory);
        var model = new Qwen3ForQuestionAnswering(config);
