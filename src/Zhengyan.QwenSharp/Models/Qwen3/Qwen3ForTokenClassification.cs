...
/// <summary>
/// Qwen3 Model with a token classification head on top (a linear layer on top of the hidden-states output).
/// e.g. for Named-Entity-Recognition (NER) tasks.
/// </summary>
public class Qwen3ForTokenClassification : nn.Module
{
    private readonly Qwen3Model _model;
    private readonly Dropout _dropout;
    private readonly Linear _score;

    public Qwen3ForTokenClassification(Qwen3Config config, string name = "model") : base(name)
    {
    ...
    public static Qwen3ForTokenClassification FromPretrained(string modelDirectory)
    {
        var config = ModelConfig.FromDirectory<Qwen3Config>(modelDirectory);
        var model = new Qwen3ForTokenClassification(config);
