...
/// <summary>
/// Qwen3 Model with a sequence classification/regression head on top (a linear layer on top of the pooled output).
/// e.g., for GLUE tasks.
/// </summary>
public class Qwen3ForSequenceClassification : nn.Module
{
    private readonly Qwen3Model _model;
    private readonly Linear _score;

    public Qwen3ForSequenceClassification(Qwen3Config config, string name = "model") : base(name)
    {
    ...
    public static Qwen3ForSequenceClassification FromPretrained(string modelDirectory)
    {
        var config = ModelConfig.FromDirectory<Qwen3Config>(modelDirectory);
        var model = new Qwen3ForSequenceClassification(config);
